...
@implementation CMBBreedNetworkClient

//Shared instance
+ (CMBBreedNetworkClient *)sharedController
{
    static CMBBreedNetworkClient *sharedController = nil;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        sharedController = [CMBBreedNetworkClient new];
    });
...
- (void)fetchAllBreeds: (void (^) (NSArray *))completion
{
    ...
    [[[NSURLSession sharedSession]dataTaskWithURL:breedURL completionHandler:^(NSData * _Nullable data, NSURLResponse * _Nullable response, NSError * _Nullable error) {
        ...
        for (id key in messageDictionary)
        {
            ...
            {
                CMBSubBreed *subBreed = [[CMBSubBreed alloc] initWithName:name imageURLs:[[NSMutableArray alloc] init]];
                [subBreeds addObject:subBreed];
            }
            //create breeds from keys on dictionary
            CMBBreed * breed = [[CMBBreed alloc] initWithName: key subBreeds: subBreeds imageURLs:[[NSMutableArray alloc] init] ];
